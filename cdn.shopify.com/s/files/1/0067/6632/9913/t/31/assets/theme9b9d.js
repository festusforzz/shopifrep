...
PaloAlto.initStickyNav = function() {
  ...
  if(position == 'fixed'){
    $('#shopify-section-header').stickybits();
    $('.main-content').css('marginTop','0');
  } else {
    //Determine what is the first
    var firstSection = $('.main-content').children().first();
    if($(firstSection).find('.slider-text-block').length != 0 || $(firstSection).find('.text-inside').length != 0 || $(firstSection).find('.featured-image-section-inner').length != 0){
      var fullHeader = true;
    } 
    //Initialize header
    if(fullHeader != true ){
      ...
      $('.main-content').css('marginTop',height);
       ...
       $('.main-content').css('marginTop','auto');
    }
...
PaloAlto.initTags = function(){
  $('.has-tag').each(function() {
   ...
   $(image).on('lazyloaded', function(e){
...
PaloAlto.initAccessibleDropdowns = function(){
  ...
  $('.mobile-nav__has-sublist').each(function(){
   ...
   //Subsub menus
  $('.has-sublist .mobile-nav__toggle').click(function(){
  ...
  $('.site-nav--has-dropdown').mouseenter(function(){
    ...
    checkAriaStatus(dropdown);
    var subsub = $(this).find('.sub-sub-links');
  });
...
PaloAlto.initSections = function() {
  ...
  if($('body').hasClass('quick-buy-enabled')){
    sections.register('featured-collection', PaloAlto.QuickBuyPopup($('.quick_buy_button')));
  }
...
PaloAlto.ProductFeatures = (function() {
  ...
  PaloAlto.Slider = (function() {
    ...
    function Slider(container) {
      ...
      function initSlider(slider, autoplay) {
        $(slider).flickity({
            autoPlay: autoplay,
            wrapAround: true,
            pageDots: false,
...
PaloAlto.initProductTabs = function() {

    $('.p-tab').not("#tab-1").addClass('hide');
  ...
  function FeaturedImage(container) {
    ...
    if($(container).hasClass('original-height')){
      ...
      var checkImageLoaded = setInterval(function() {
        if ( $(container).hasClass('lazyloaded') ) {
        proportionize(container);
    ...
    function proportionize(container){
      ...
      actualImage.onload = function() {
        ...
        var factor = 1 - ((imageWidth - windowWidth)/(imageWidth));
        var newHeight = factor*imageHeight;
    ...
    function setHeights(container){
      ...
      if(!$(content).hasClass('onboarding-blog-content')){
        ...
        $(content).each(function(){
          ...
          var dateHeight = $(this).find('.h6').height();
        ...
        $('.featured-blog__content').css('height', tallestContent);
      }
    }
...
timber.productPage = function (options) {
  ...
  if (variant) {
    ...
    if (variant.available) {
      ...
      document.addEventListener('shopify:payment_button:loaded', checkButtons(btn));
      function checkButtons(btn){
        var sibling = $(btn).next();
    ...
    $quantityElements.hide();
    document.addEventListener('shopify:payment_button:loaded', checkButtons($addToCart));
      function checkButtons(btn){
        var sibling = $(btn).next();
        if($(sibling).hasClass('shopify-payment-button')){
              $(sibling).hide();
...
// HC - Sticky Header on scroll - 26 Jun '18
$(window).scroll(function () {
  var header = $("#shopify-section-header");
  if ( $(this).scrollTop() > 200 && !header.hasClass('fixed-header') ) {
    header.addClass('fixed-header');
    header.find(".site-header").addClass("fixed color-fix");
  } 
  else if ( $(this).scrollTop() <= 200 ) {
    header.removeClass('fixed-header');
    //HC-Leave fixed class even after scroll-25 Sep'18
    header.find(".site-header").removeClass("fixed color-fix");
  }
});

// HC - About Section position and resize - 4 Jul '18
$(document).ready(function() {
  const bannerHeight = $('.about-banner').outerHeight() - 50;
  $("#shopify-section-about").css({
